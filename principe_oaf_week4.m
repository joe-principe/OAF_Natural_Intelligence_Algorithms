% Script file: principe_oaf_week4.m
%
% Define variables:
...
%   data              -- The iris data as a 1x5 cell array
...
%                        particles, columns are coordinates
...
%                        particles, columns are velocities
...
for ii = 1:3
    ...
    % Done in this for loop because new particles are needed for each centroid
    [P_sl, P_sw, P_pl, P_pw] = set_starting_pos(sepal_length, sepal_width, petal_length, petal_width, num_particles);
    ...
    p_best = P;
    fit_p_best = fit_func(P, measurements, num_entries, ii);
...
fprintf('The centrosa   centroid is located at: (%.4f, %.4f, %.4f, %.4f)\n',...
        centroids(1, 1), centroids(1, 2), centroids(1, 3), centroids(1, 4));
fprintf('The versicolor centroid is located at: (%.4f, %.4f, %.4f, %.4f)\n',...
        centroids(2, 1), centroids(2, 2), centroids(2, 3), centroids(2, 4));
fprintf('The virginica  centroid is located at: (%.4f, %.4f, %.4f, %.4f)\n',...
        centroids(3, 1), centroids(3, 2), centroids(3, 3), centroids(3, 4));

% Calculates the distance of each particle from each data point in 4D space
function dist = fit_func(particles, datapoints, num_entries, val)
    % Initialize the distance matrix
    % Size will be (rows of particles matrix, 1)
    dist = zeros(size(particles, 1), 1);
    ...
    for ii = 1:size(particles, 1)
        ...
        for jj = num_entries * (val - 1) + 1 : num_entries * val
            % Calculate the euclidean distance from particle to datapoint
            dist(ii, 1) = sqrt( (particles(ii, 1) - datapoints(jj, 1)).^2 ...
...
function [w, x, y, z] = set_starting_pos(sepal_length, sepal_width, petal_length, petal_width, num_particles)
